...
import com.xixiha.hook.BuildConfig.CHANNEL_ID
import com.xixiha.hook.BuildConfig.SERVICE_NAME
import com.xixiha.hook.R
import com.xixiha.hook.ui.activity.MainActivity
import com.xixiha.hook.utils.Data
import com.xixiha.hook.utils.Data.getBoolean
import com.xixiha.hook.utils.Data.getConfig
import com.xixiha.hook.utils.Data.getInt
import com.xixiha.hook.utils.Data.isAccessibilitySettingsOn
import com.xixiha.hook.utils.Data.skipCount
import com.xixiha.hook.utils.FindId
import com.xixiha.hook.utils.Language
...
class SkipServiceImpl(private val service: SkipService) {
    ...
    init {
        ...
        notification = if (context.isAccessibilitySettingsOn(SERVICE_NAME)) {
            createNotification(
                context.getString(R.string.accessibility_notification).format(skipCount)
            )
        } else {
        ...
        if (context.isAccessibilitySettingsOn(SERVICE_NAME) && !show)
            Toast.makeText(
                context,
                context.getString(R.string.service_open_success),
                Toast.LENGTH_SHORT
            ).show()
        show = true
    }
    ...
    private fun success() {
        ...
        if (context.getConfig().getBoolean(Data.hookTip.key, true))
            Toast.makeText(
                context,
                context.getString(R.string.tip_skip_success),
                Toast.LENGTH_SHORT
            ).show()
        refresh()
    }
    ...
    fun onInterrupt() {
        ...
        Toast.makeText(
            context,
            context.getString(R.string.close_accessibilityservice),
            Toast.LENGTH_SHORT
        ).show()
        service.startForeground(1, notification)
    }
    ...
    private fun findSkipButtonByText(nodeInfo: AccessibilityNodeInfo): Boolean {
        ...
        if (list.isNotEmpty()) {
            for (node in list) {
                ...
                if (className == "android.widget.textview" || className.toLowerCase(Locale.getDefault())
                        .contains("button")
                ) {
                    if (text == "跳过" || text == "跳过广告" || textRegx1.matches(text) || textRegx2.matches(
                            text
                        ) || textRegx3.matches(text)
                    ) {
                        if (time != eventTime && eventTime - time > 800) {
                            skip(node)
                            time = eventTime
