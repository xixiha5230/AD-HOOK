package com.xixiha.hook.ui.fragment
...
import com.xixiha.hook.databinding.FragmentMainBinding
import com.xixiha.hook.ui.activity.MainActivity.Companion.searchText
import com.xixiha.hook.ui.adapter.ListViewAdapter
import com.xixiha.hook.utils.AppInfo
...
class MainFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        refresh.setOnRefreshListener {
            if (searchText.isEmpty()) {
                ...
                for (app in this.list) {
                    if (app.app_name.contains(searchText) or app.app_package.contains(searchText)) list.add(
                        app
                    )
                }
                showList(list)
        ...
        listView.setOnItemLongClickListener { _, view, i, _ ->
            this.position = i
            listView.showContextMenu(view.x, view.y)
            true
        }
    }
    ...
    fun showList(list: ArrayList<AppInfo>) {
        Thread {
            val instance =
                Collator.getInstance(Language.fromId(appContext.modulePrefs.get(Data.localeId)))
            try {
                /** 排序 */
